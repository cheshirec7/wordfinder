(function(exports, $){
  $( document ).ready(function() {
    ...
    var $window = $(window).on('resize', function(){
      ...
      }).ajaxStop(function() { //called when all Ajax requests have completed
        $searchSpinner.hide().removeClass('is-active');
        $btnSubmit.attr('disabled', false);
      }),
      ...
      $formSearch = $( "#formSearch" ).submit(function( event ) {
        ...
        switch(backend) {
          case 0: url = 'http://gowordfinder.appspot.com/find'; break;
          case 1: url = 'http://nodewordfinder.herokuapp.com/find'; break;
          case 2: url = 'http://phpwordfinder.erictotten.info/find'; break;
          case 3: url = 'http://pywordfinder.appspot.com/search'; break;
        }
    ...
    function validateInput($traytext) {
      ...
      for (var i=0, j=$traytext.length; i < j; i++) {
        if ($traytext[i] == '?')
          result.wild_count++;
        else {
      ...
      for (var i=0;i < result.wild_count; i++)
        result.tray += '?';
